...
def get_metrics(output_folder, sequence_name, target_length=621): # the EGFR sequence has 621 residues
    ...
    binder_len = len(pae) - target_length # note that we are assuming that the binder always comes first in the sequence!
    pae_interaction = (pae[: binder_len, binder_len:].mean() + pae[binder_len:, : binder_len].mean()) / 2
    scores = None
    for file in files:
...
@click.command()
@click.option("--target_length", default=621, help="Length of the target protein (in amino acids)")
